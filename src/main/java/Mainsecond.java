...
//                System.out.println("true");
        ...
        for (Map.Entry<Integer, List<String>> entry : students.entrySet()) {
            ...
            for (Map.Entry<Integer, List<String>> innerEntry : students.entrySet()) {
                ...
                if (match_multiplesubject(currentSubjects, innerSubjects)) {
                    int length = 0;
                    for (Integer member : group) { //compare studentid of current group memeber to innerstudentid
                        if (member == studentId) // except studentId taking currentSubject
                            continue;
                        if (match_multiplesubject(students.get(member), innerSubjects))
                            length++;
                    }
        ...
        //special matching -> comparing one subject
        for (Map.Entry<Integer, List<String>> entry : students.entrySet()) {
            int studentId = entry.getKey();
            if (visitedStudents.contains(studentId)) {
                continue; // Skip if the student is already assigned to a group
            ...
            for (Map.Entry<Integer, List<String>> innerEntry : students.entrySet()) {
                ...
                if (match_onesubject(currentSubjects, innerSubjects)) {
                    int length = 0;
                    for (Integer member : group) { //compare studentid of current group memeber to innerstudentid
                        if (member == studentId) // except studentId taking currentSubject
                            continue;
                        if (match_onesubject(students.get(member), innerSubjects))
                            length++;
                    }
                    if (length == group.size() - 1) {//except studentId taking currentSubject
                        visitedStudents.add(innerStudentId);
...
//        int groupNumber = 1;
        System.out.println();
        System.out.println("after second matching and special matching withtout friend group");
...
//        int groupNumber = 1;
    ...
    static boolean match_multiplesubject(List<String> currentSubjects, List<String> innerSubjects) {
    ...
    static boolean match_onesubject(List<String> currentSubjects, List<String> innerSubjects) {
